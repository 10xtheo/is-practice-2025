export const validationMethodsWithoutParams = {
	...
	isNumber(value: string) {
		...
		return !regex.test(value)
			? { status: false, errorMessage: 'Поле должно содержать только цифры' }
			: { status: true };
	},
	isNumberWithColon(value: string) {
		let regex = /^[0-9:]+$/gi;
		return !regex.test(value)
			? { status: false, errorMessage: 'Поле должно содержать цифры и двоеточия' }
			: { status: true };
	},
...
export const validationMethodsWithParams = {
	minLength(value: string, minLength: number) {
		return value.length < minLength
			? { status: false, errorMessage: `Поле должно содержать не менее ${minLength} символов` }
			: { status: true };
	},
	maxLength(value: string, maxLength: number) {
		return value.length > maxLength
			? { status: false, errorMessage: `Поле должно содержать не более ${maxLength} символов` }
			: { status: true };
	},
...
export const validationMethodsWithDates = {
	isDateInFuture(date1: number, date2: number) {
		return date2 - date1 > 0
			? { status: false, errorMessage: 'Дата окончания не может быть раньше даты начала' }
			: { status: true };
	},
};
