import {
	METHODS,
	RequestsOptionsEvents,
	RequestsOptionsCalendars,
	RequestsOptionsUsers,
	RequestsOptionsEventParticipants,
	RequestsOptionsCalendarParticipants,
} from './types';
...
class HttpEventParticipants {
	...
	post = async <IDto>(url: string, body: IServerUserParticipant) =>
		this.makeRequest<IDto>({ url, method: METHODS.POST, body });
	delete = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.DELETE });
	put = async <IDto>(url: string, body: Omit<IServerUserParticipant, 'user_id'>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PUT, body });
}
...
class HttpCalendarParticipants {
	...
	post = async <IDto>(url: string, body: IServerUserCategoryParticipant) =>
		this.makeRequest<IDto>({ url, method: METHODS.POST, body });
	delete = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.DELETE });
	put = async <IDto>(url: string, body: Omit<IServerUserCategoryParticipant, 'user_id'>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PUT, body });
}
...
class HttpEvents {
	...
	post = async <IDto>(url: string, body: IEventCreate) =>
		this.makeRequest<IDto>({
			url,
			method: METHODS.POST,
			body: {
				title: body.title,
	...
	postTimeManagement = async <IDto>(url: string, body: IEventTimeManagement) =>
		this.makeRequest<IDto>({ url, method: METHODS.POST, body: body });
	delete = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.DELETE });
	patch = async <IDto>(url: string, body: Partial<IEvent>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PATCH, body });
	put = async <IDto>(url: string, body: Partial<IEvent>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PUT, body });
}
...
class HttpCalendars {
	...
	get = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.GET });
	post = async <IDto>(url: string, body: ICalendarCreate) =>
		this.makeRequest<IDto>({
			url,
			method: METHODS.POST,
			body: { category_in: { title: body.title, participants: body.participants }, event_ids: [] },
		});
	delete = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.DELETE });
	patch = async <IDto>(url: string, body: Partial<ICalendar>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PATCH, body });
	put = async <IDto>(url: string, body: Partial<ICalendar>) =>
		this.makeRequest<IDto>({ url, method: METHODS.PUT, body: { category_in: { title: body.title } } });
}
...
class HttpUsers {
	...
	get = async <IDto>(url: string) => this.makeRequest<IDto>({ url, method: METHODS.GET });
	patch = async <IDto>(url: string, body: IUserUpdate) =>
		this.makeRequest<IDto>({ url, method: METHODS.PATCH, body });
}
