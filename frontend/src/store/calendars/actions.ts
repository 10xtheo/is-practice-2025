...
export const getCalendars = createAsyncThunk<ICalendar[], void, { rejectValue: string }>(
	...
	async (_, { rejectWithValue }) => {
		try {
			...
			return Promise.all(
				calendars?.data.map(async (calendar) => {
...
export const updateCalendar = createAsyncThunk<
	...
	{ rejectValue: string }
>('calendars/updateCalendar', async ({ calendarId, calendarData }, { rejectWithValue }) => {
	try {
...
>('calendars/calendar-add-participant', async ({ calendarId, participant }, thunkAPI) => {
	try {
...
export const deleteCalendarParticipant = createAsyncThunk<string, { calendarId: string; userId: string }>(
