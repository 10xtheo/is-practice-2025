...
const Profile: FC<ProfileProps> = ({ currentUser }) => {
	...
	useEffect(() => {
		if (editedUser && currentUser) {
			const hasChanges = editedUser.full_name !== currentUser.full_name || editedUser.email !== currentUser.email;
		...
		<div className="profile">
			<div className="profile__container">
				...
				<div className="profile__actions">
					{!isEditing ? (
						<>
							<button className="profile__edit-button" onClick={() => setIsEditing(true)}>
								Редактировать
							</button>
							<button
