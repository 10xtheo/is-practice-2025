			...
			{files.length === 0 ? (
				...
				files.map((file) => (
					<div key={file.id} className={styles.file}>
						<a href={file.file_url} target="_blank" rel="noopener noreferrer" className={styles.file__link}>
							...
							<div className={styles.file__info}>
								...
								<div className={styles.file__user}>
									Добавил:{' '}
									{users.find((u) => u.id === file.user_id)?.full_name || 'Неизвестный пользователь'}
								</div>
							</div>
						</a>
						<button
