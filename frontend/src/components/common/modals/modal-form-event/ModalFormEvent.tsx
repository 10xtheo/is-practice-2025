...
import {
	...
	getEventRepeatTypeOptions,
	getEditEventRepeatTypeOptions,
} from '../helpers';
...
const ModalFormEvent: FC<IModalFormEventProps> = ({
	...
	const onSubmit: TSubmitHandler<IModalValues> = async (data) => {
		...
		if (data.listeners) {
			users
				.filter((user) => data.listeners.includes(user.id))
		...
		allParticipants.splice(
			allParticipants.findIndex((u) => u.id === currentUser.id),
			1,
		);
		...
		try {
			...
			const slots = await dispatch(
				findAvailableTimeSlots({
					...
					start_date: new Date(
						values.start - new Date(values.start).getTimezoneOffset() * 60 * 1000,
					).toISOString(),
					end_date: new Date(
						values.end - new Date(values.end).getTimezoneOffset() * 60 * 1000 + 24 * 60 * 60 * 1000,
					).toISOString(),
				}),
			).unwrap();
			...
			<div className={styles.modal} ref={modalRef}>
				<div className={styles.modal__content}>
					...
					<form className={styles.modal__form} onSubmit={handleSubmit(onSubmit)}>
						...
						{(!!errors.start || !!errors.end) && (
							<div className={styles.modal__form__error}>{errors.start ?? errors.end}</div>
						)}
						...
						<div className={cn(styles.modal__form__group)}>
							...
							<UserMultiSelector
								defaultSelectedUsers={
									defaultEventValues.participants.length > 0
                ...
                "
						...
						<div style={{ display: isRecurring ? 'block' : 'none' }}>
							...
							<div
								className={cn(styles.modal__form__group)}
