...
const LongEvent: FC<ILongEventProps> = ({
	...
	return (
		...
		>
			<div className={styles.event__container} style={eventContainerStyle}>
				{event.title}
				{event.type === 'event' && `, ${formatDate(new Date(event.start), 'hh:mm')}`}
				{isMovingFromPrev && (
					<div className={cn(styles.event__arrow, styles.event__arrow_left)} style={arrowLeftStyle}></div>
				)}
				{isMovingToNext && (
					<div className={cn(styles.event__arrow, styles.event__arrow_right)} style={arrowRightStyle}></div>
				)}
			</div>
		</div>
	);
