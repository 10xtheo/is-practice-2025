			...
			if (debouncedSearchQuery.trim()) {
				try {
					...
					const response = await axios.get(
						`http://localhost:8000/api/v1/basic-search?q=${encodeURIComponent(debouncedSearchQuery)}`,
						{
							headers: headers,
						},
					);
	...
	return (
		<header className={styles.header}>
			...
			<div className={styles.navigation}>
				<button className={cn(styles.navigation__today__btn, 'button')} onClick={changeToToday}>
					Сегодня
				</button>
				<div className={styles.navigation__body}>
					<div className={styles.navigation__icons}>
						<button className={cn('icon-button', styles.navigation__icon)} onClick={changeToPrev}>
							<i className="fas fa-chevron-left"></i>
						</button>
						<button className={cn('icon-button', styles.navigation__icon)} onClick={changeToNext}>
			...
			<Select onChangeOption={handleModeChange} options={modes} selectedOption={selectedOption} />
			<button
