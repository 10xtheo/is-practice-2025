...
const SearchDropdown: React.FC<SearchDropdownProps> = ({ results, onEventClick, onCategoryClick, onUserClick }) => {
			...
			{results.events.length > 0 && (
				<div className={styles.section}>
					...
					{results.events.map((event) => (
						<div key={event.id} className={styles.item} onClick={() => onEventClick(event.id)}>
			...
			{results.categories.length > 0 && (
				<div className={styles.section}>
					...
					{results.categories.map((category) => (
						<div key={category.id} className={styles.item} onClick={() => onCategoryClick(category.id)}>
			...
			{results.users.length > 0 && (
				<div className={styles.section}>
					...
					{results.users.map((user) => (
						<div key={user.id} className={styles.item} onClick={() => onUserClick(user.id)}>
