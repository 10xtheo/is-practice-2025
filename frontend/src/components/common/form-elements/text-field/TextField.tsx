...
const TextField = forwardRef<HTMLInputElement, IFieldProps>(
	({ error, type = 'text', style, className = '', isShowError = true, fullWidth = false, onClick, ...rest }, ref) => {
		...
		return (
			<div
				...
				style={style}
			>
				<div className={styles.text__field__input} onClick={handleClick}>
					<input
				...
				{error && isShowError && <div className={styles.text__field__error__text}>{error}</div>}
			</div>
		);
	},
);
